...
int8_t STORAGE_GetCapacity_FS(uint8_t lun, uint32_t *block_num, uint16_t *block_size)
{
  ...
  *block_num  = w25qxx.SectorCount; // STORAGE_BLK_NBR;
  *block_size = w25qxx.SectorSize; // STORAGE_BLK_SIZ;
  return (USBD_OK);
  /* USER CODE END 3 */
}
...
int8_t STORAGE_Read_FS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  ...
  // Please refer: https://controllerstech.com/stm32-usb-msc/
  return (USBD_OK);
  /* USER CODE END 6 */
}
...
int8_t STORAGE_Write_FS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  ...
  for(int32_t i = 0; i < blk_len; i++)
  {
	  ...
	  W25qxx_WriteSector(buf + w25qxx.SectorSize*i, blk_addr+i, 0, 0);
  }
  return (USBD_OK);
  /* USER CODE END 7 */
}
