/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2023 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define LED_CTL_RCK_Pin GPIO_PIN_13
#define LED_CTL_RCK_GPIO_Port GPIOC
#define LED_CTL_SER_Pin GPIO_PIN_14
#define LED_CTL_SER_GPIO_Port GPIOC
#define LED3_Pin GPIO_PIN_0
#define LED3_GPIO_Port GPIOA
#define LED2_Pin GPIO_PIN_1
#define LED2_GPIO_Port GPIOA
#define LED1_Pin GPIO_PIN_2
#define LED1_GPIO_Port GPIOA
#define LED0_Pin GPIO_PIN_3
#define LED0_GPIO_Port GPIOA
#define SPI1_CS_Pin GPIO_PIN_0
#define SPI1_CS_GPIO_Port GPIOB
#define LED_CTL_OE_Pin GPIO_PIN_2
#define LED_CTL_OE_GPIO_Port GPIOB
#define ROW_1_Pin GPIO_PIN_12
#define ROW_1_GPIO_Port GPIOB
#define ROW_2_Pin GPIO_PIN_13
#define ROW_2_GPIO_Port GPIOB
#define ROW_3_Pin GPIO_PIN_14
#define ROW_3_GPIO_Port GPIOB
#define ROW_4_Pin GPIO_PIN_15
#define ROW_4_GPIO_Port GPIOB
#define ROW_5_Pin GPIO_PIN_8
#define ROW_5_GPIO_Port GPIOA
#define LED_C8_Pin GPIO_PIN_9
#define LED_C8_GPIO_Port GPIOA
#define LED_R8_Pin GPIO_PIN_10
#define LED_R8_GPIO_Port GPIOA
#define LED_CTL_SCK_Pin GPIO_PIN_15
#define LED_CTL_SCK_GPIO_Port GPIOA
#define COL_1_Pin GPIO_PIN_3
#define COL_1_GPIO_Port GPIOB
#define COL_2_Pin GPIO_PIN_4
#define COL_2_GPIO_Port GPIOB
#define COL_3_Pin GPIO_PIN_5
#define COL_3_GPIO_Port GPIOB
#define COL_4_Pin GPIO_PIN_6
#define COL_4_GPIO_Port GPIOB
#define COL_5_Pin GPIO_PIN_7
#define COL_5_GPIO_Port GPIOB
#define B0_Pin GPIO_PIN_8
#define B0_GPIO_Port GPIOB
#define B0_EXTI_IRQn EXTI9_5_IRQn
#define B1_Pin GPIO_PIN_9
#define B1_GPIO_Port GPIOB
#define B1_EXTI_IRQn EXTI9_5_IRQn

/* USER CODE BEGIN Private defines */
#define FLASH_CS_GPIO_Port SPI1_CS_GPIO_Port
#define FLASH_CS_Pin SPI1_CS_Pin
/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
